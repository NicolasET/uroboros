...
**B. Dispatch the reviewer.** Use the Task tool to call the `uroboros-reviewer` subagent in the **foreground**. First read the `model:` and `effort:` declared in `agents/uroboros-reviewer.md`'s frontmatter, then pass that same model **explicitly on the Agent call** — the frontmatter is the single source of truth for the reviewer's model, but the call-level parameter is what reliably takes effect (it takes precedence and guards against the frontmatter `model` being ignored in some versions). To change the reviewer's model/effort, users edit only the frontmatter. Put in its prompt:
...
1. **BLOCKING — Ask the user, via `AskUserQuestion`, which model and which reasoning effort to use for the implementer this run** (e.g. model: Fable 5 / Opus 4.8 / Sonnet 5 / other; effort: high / xhigh / max). This is a per-run choice — do not hardcode it, and do not dispatch the implementer until the user has answered *in this run*. On a resume, if `loop-state.md` records a prior choice, re-confirm it rather than silently reusing it. Record the chosen model+effort in `loop-state.md`.
