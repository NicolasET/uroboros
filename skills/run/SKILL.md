...
| Flag | Effect |
|---|---|
| `--auto` | **Never call `AskUserQuestion` — except once, at the very start, for the model/effort choices** (Model/effort protocol). Every other would-be question (intake, findings, risks, approvals) is resolved by the assumption mechanism below. Intake approval is skipped: log the prompt as self-approved (`A<n>`). Review rounds are **uncapped** unless `--rounds=N` is given (step D). |
...
| `--rounds=N` | Max review rounds per phase (default 3; under `--auto` the default is no cap). In goal mode it also caps the Stop hook's automatic relaunches. |
...
**Assumption mechanism (the only lawful way to suppress a question).** When the active mode suppresses a question, you still never guess silently. Instead: (1) choose the most conservative resolution — smaller scope, no new user-visible behavior beyond the idea, reversible over irreversible; (2) record it as `A<n>` in the DECISION LOG and in `loop-state.md` under `ASSUMPTION LOG` with a one-line rationale; (3) reflect it in the artifact where a clarification would have gone (e.g. `## Clarifications` entry marked `(assumed — A<n>)`); (4) carry the full assumption ledger into the Loop Report for the user to audit. The model/effort choices are never suppressed this way — under `--auto` they are asked up front (Model/effort protocol).
...
1. **You and B never infer product/design decisions silently.** Every ambiguity becomes an `AskUserQuestion` to the user — or, when the active flags suppress it, a recorded assumption per the mechanism above. The user only ever interacts through your `AskUserQuestion` prompts and the intake gates (shared-understanding check, prompt approval) — never at a tool/hook boundary.
...
Both subagents run on a model + effort the user chooses at runtime. If the corresponding flag (`--reviewer=` / `--implementer=`) was given, that IS the user's choice — record it and do not ask. Otherwise: one blocking `AskUserQuestion` call per choice, containing **two separate questions**: question 1 = the model family (options: Fable, Opus, Sonnet — each described as the latest release of that family — plus free-form "Other"; never put version numbers in the options, they go stale); question 2 = the effort (options: high, xhigh, max). Never combine model and effort into one question's options, and never label any option as recommended or default. Dispatch with the family alias (`fable`, `opus`, `sonnet`, `haiku`), which resolves to the latest release of that family; if the user names an exact version, record it as given and dispatch its family alias.
...
- **Under `--auto`:** an `--auto` run has no later chance to ask, so both choices are settled **at the very start** — right after parsing the flags, before the compat check and intake (on a resume, right after restoring the recorded state). Make one blocking `AskUserQuestion` call holding the questions for every choice whose flag is missing: reviewer model, reviewer effort, implementer model, implementer effort (up to 4 questions, same options as above). This is the only `AskUserQuestion` an `--auto` run makes. The later settle points (intake step 7, the implement protocol, the goal protocol) find the choice already recorded and do not ask again. Only if `AskUserQuestion` is unavailable (e.g. a headless `-p` session) fall back to each agent's frontmatter model/effort, recorded as `A<n>`. Until `.uroboros/intake.md` exists, hold the answers and write them into it when it is created.
- Record every choice in the DECISION LOG and `loop-state.md`. On a resume, re-confirm the recorded choice instead of silently reusing it (in `--auto`, reuse it and note the reuse in the log; a choice not yet recorded is asked at the start of the resume, per the bullet above). If a dispatch is due and no choice is recorded, apply the protocol now (once) — never substitute a model of your own outside the assumption mechanism.
...
4. **Interrogate in rounds** with `AskUserQuestion` (questions in the user's language, per the Question protocol). A round is the **frontier**: every decision whose prerequisites are already settled — nothing else. A question whose answer hinges on another question still open belongs to a later round, never the same one. Within the frontier, **ask first what would change the architecture or the data model**; split a frontier larger than 4 across consecutive calls. After each round, update the tree and `.uroboros/intake.md`, recompute the frontier, and ask the next round; later rounds must ask what earlier answers unblocked, not a pre-written list. Intake ends when the frontier is empty: every branch visited, nothing silently assumed.
   - **Facts are yours, decisions are the user's.** When a question needs a fact the repo can answer (does X exist, how is Y done today, what does Z return), look it up with Grep/Glob/Read instead of asking. Only the questions that depend on that fact wait for it; ask the rest of the frontier now.
   - **Invite references.** In the first round, ask whether existing code, a mockup, or a library already does something close to what they want — source code is the highest-fidelity spec input; read whatever they point at and carry it into the specify prompt as explicit references.
   - **"I don't know" is a valid answer.** Offer it as an option whenever the user may lack the answer. Record the decision as **DEFERRED** (plain description + why it is open); it is not an assumption and you never fill it in. The specify prompt carries it as an explicit open decision (step 6), so specify marks it `[NEEDS CLARIFICATION]` and it reaches the user again at the review of the spec.
   - **Ungrillable questions** — how something should look or feel, a layout, an interaction — cannot be settled by talking. Do not rephrase them round after round: ask once for a mockup, screenshot or reference to react to; if there is none, record the decision as DEFERRED.
   - *(Under `--auto`: run the same rounds against yourself and resolve every question via the assumption mechanism — nothing is deferred. Under `--only-business`: still ask the business questions — user-visible failure behavior and edge cases are business; assume the technical ones.)*
5. **Shared-understanding check.** Before drafting the prompt, show the user, in their language, one summary: the decisions settled (plain words), the non-goals, and the DEFERRED decisions. Ask via `AskUserQuestion` whether it matches what they want (Matches / Something is off). If something is off, reopen the affected branches and run more rounds. *(Skipped under `--auto` — record that it was skipped.)*
6. Draft a WHAT/WHY-focused **English** prompt for specify (no tech/implementation), citing any references from step 4, listing the non-goals, and ending with an **Open decisions** section that names every DEFERRED item and says: "Do not resolve these; mark each `[NEEDS CLARIFICATION]`." (Specify caps markers at 3; any beyond land in its Assumptions, which the specify review relays to the user — nothing is lost.) Show it and get approval via `AskUserQuestion` (Approve / Edit). Revise until approved. **Approval required** *(skipped under `--auto` — self-approve and record `A<n>`)*.
7. **BLOCKING — settle the reviewer's model/effort per the Model/effort protocol.** Do not dispatch the reviewer anywhere in the run before this is settled. *(Under `--auto` it was settled at the start of the run.)*
...
Then create `FEATURE_DIR/loop-state.md` with: the feature/branch, the ACTIVE FLAGS, the approved English prompt, and — copied from `.uroboros/intake.md` — the DECISION LOG so far (intake answers), the ASSUMPTION LOG so far (if any) and the DEFERRED list, plus an empty per-phase section for specify → implement. Delete `.uroboros/intake.md` once the copy is written. Update `loop-state.md` at every step below.
...
- Re-run the gate (A2) if code/artifact changed, then re-dispatch B (step B) to verify. Repeat C–D until CLEAN-with-evidence + green gate, max **3** rounds (or `--rounds=N` if given); if still not clean, surface the remaining items to the user plainly — each one described per the Question protocol, never listed by id — and let them decide to accept or stop. Record the outcome in `loop-state.md`.
- **Round cap under `--auto`.** Without `--rounds`, there is **no cap**: repeat C–D until CLEAN-with-evidence (+ green gate for implement), however many rounds it takes — the run stops only on that, an error, or the user interrupting. With an explicit `--rounds=N`, when the cap is exhausted: on a design phase (specify, clarify, plan, tasks, analyze — and `goal.md` in goal mode) **advance** — record every resolution from the last fold that B has not verified as an `A<n>` marked `unverified`, record the exhausted cap in `loop-state.md`, and continue; on implement (and goal mode's rounds) **stop and report** — never advance code past an exhausted cap or a red gate.
