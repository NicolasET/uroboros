...
1. **Intake.** Uroboros first checks the project's spec-kit version and skills against its [compatibility contract](./references/spec-kit-compat.md) (a different major gets a question; a missing core skill stops the run), then runs a **blind-spot pass** — exploring the codebase around your idea to surface the unknown unknowns you'd never think to mention (prior work, invariants your idea touches, decisions it silently implies) — then interrogates your idea in **rounds over a decision tree**: each round asks only the decisions whose prerequisites you have already settled (architecture-changing ones first), and the next round is recomputed from your answers. It covers goal, scope in/out and non-goals, entities, done-criteria, failure behavior and edge cases; looks facts up in the repo instead of asking you; offers to split an idea that is really several features; accepts "I don't know" (the decision is carried to specify as an open `[NEEDS CLARIFICATION]`, never filled in); and asks for a mockup or reference when a question is about look and feel. It also invites references: existing code, mockups, or libraries close to what you want are the highest-fidelity spec input. It then shows a summary of what it understood (settled, non-goals, deferred) for you to confirm, drafts an English `specify` prompt and asks for your approval. Intake progress is saved to `.uroboros/intake.md` after every round, so an interrupted intake resumes where it stopped.
...
- **`.uroboros/`:** holds the transient `intake.md` draft while an intake is in progress (both modes; deleted once the run's `loop-state.md` exists) and, in goal mode, `goal.md` + `loop-state.md` per feature and the transient `active-run.json` marker the Stop hook reads. Commit the per-feature files if you want run history; `intake.md` and `active-run.json` are safe to gitignore.
