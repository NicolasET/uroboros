...
- The spec-kit version detected at the start of the run (the `SPEC-KIT:` line in `loop-state.md`) and the contract version the plugin was verified against.
- **Models and guides:** per role (orchestrator, reviewer, implementer), the release it ran on and the model guide applied or `none` (the `MODEL GUIDES` record in `loop-state.md`), plus every model mismatch the Model check caught and how it was resolved.
- Per phase: inferences caught, questions answered, risks accepted, gate result — and, for implement, the convergence outcome per round (converged, or the appended task ids).
